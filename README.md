...
정확도를 깨뜨리는 흔한 실패(문서 매핑표가 코드와 어긋남·이중 카운트·비밀 노출·비가역 콘솔 설정 누락 등)를 **G1–G10 규칙**으로 못박았습니다. 특히 GA4 콘솔의 **비가역 설정**(BigQuery 링크 백필 없음·데이터 보존·맞춤 측정기준·트래픽 필터)은 **트래픽 시작 전**에 처리하도록 가이드 맨 앞 콜아웃으로 강조합니다.
