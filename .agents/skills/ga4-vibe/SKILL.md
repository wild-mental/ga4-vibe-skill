...
1. 연동 대상 앱이 이미 이벤트 로깅·분석 대시보드를 가졌는지, 그 코드베이스 경로 — Phase A 스캔 대상
2. GA4(+BigQuery)·플랫폼·전송 방식 등 갈림길이 모호하면 추측하지 말고 자료 요청 또는 객관식 결정 질문(Phase A 1.3)
3. 비밀값(MP `api_secret`·서비스계정 키)을 대화 맥락에 유입시키지 않는다

확인은 1턴에 묶어 처리한다. Phase A 충분성 게이트를 통과하기 전에는 구현을 착수하지 않는다.
...
정확도를 깨뜨리기 쉬운 실패를 **착수 전 규칙**으로 못박는다. 위반은 Phase D 평가 전에 스스로 잡는다.
